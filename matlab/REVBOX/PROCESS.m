...
%nAlpha = length(alpha);
alpha = [0, alpha];       
sAlpha = alpha >= 0; % & alpha <= 5;
Table = zeros(sum(sAlpha), 7, nFRAC); %sloupce alpha, se, sD, D, D-D_theoretical
bestAlpha = zeros(nAlpha,1);

for k=1:nFRAC  
    cellH = load(['cellH' num2str(k) '.mat']);
    cellH = cellH.cellH;
